...
const SettingsScreens = defineComponent({
  ...
  setup(props) {
     ...
     const isAdmin = authStore.user?.roles?.some((r: string) => ['admin', 'moderator'].includes(r))
     return () => h('div', { class: 'pv-settings-grid' }, [
      h('aside', { class: 'pv-settings-nav' }, [h('small', 'ACCOUNT SETTINGS'), ...tabs.map(tab => h(RouterLink, { to: tab[1], class: props.page === tab[0] ? 'active' : '' }, () => [h(PvIcon, { name: tab[3] }), tab[2]])), isAdmin ? h(RouterLink, { to: '/admin', class: 'pv-admin-link' }, () => [h(PvIcon, { name: 'shield' }), 'Admin Panel']) : null]),
