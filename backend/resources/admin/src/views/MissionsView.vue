  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="relative flex-1 max-w-md">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search missions..." @input="debouncedSearch" class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all" />
      </div>
        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Objective</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Target</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Cash Reward</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.objective_type }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.objective_count }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${(item.cash_reward || 0).toLocaleString()}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ (item.experience_reward || 0).toLocaleString() }}</td>
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Mission</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div class="col-span-2">
                ...
                <input v-model="formData.name" type="text" required placeholder="e.g. First Blood" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-300 mb-2">Description</label>
                <textarea v-model="formData.description" rows="3" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="What the player needs to do..."></textarea>
              </div>
              <div>
                ...
                <select v-model="formData.type" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                  <option v-for="opt in missionFrequencies" :key="opt.name" :value="opt.name">{{ opt.label }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Objective Type</label>
                <select v-model="formData.objective_type" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                  <option v-for="opt in missionObjectiveTypes" :key="opt.name" :value="opt.name">{{ opt.label }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Objective Count</label>
                <input v-model.number="formData.objective_count" type="number" required min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">XP Reward</label>
                <input v-model.number="formData.experience_reward" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Respect Reward</label>
                <input v-model.number="formData.respect_reward" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Cooldown (hours)</label>
                <input v-model.number="formData.cooldown_hours" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
          ...
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Mission</button>
          </div>
        </div>
      </div>
    </div>
...
const missionFrequencies = ref([])
const missionObjectiveTypes = ref([])
...
onMounted(async () => {
  try {
    const [freqs, objTypes] = await Promise.all([
      api.get('/admin/mission-frequencies'),
      api.get('/admin/mission-objective-types')
    ])
    missionFrequencies.value = freqs.data
    missionObjectiveTypes.value = objTypes.data
  } catch(e) {}
  fetchItems()
})
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/missions', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/missions/${editingItem.value.id}`, formData.value)
      toast.success('Mission updated successfully!')
    } else {
      await api.post('/admin/missions', formData.value)
...
const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Mission? This action cannot be undone.', 'Delete Mission')
  if (!confirmed) return
  try {
