  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="relative flex-1 max-w-md">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search stocks..." @input="debouncedSearch" class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all" />
      </div>
        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Sector</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Price</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Shares</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Volatility %</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.symbol }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.name }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.sector }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.current_price || 0).toFixed(2)}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.shares_available }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.volatility }}</td>
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Stock</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="formData.symbol" type="text" required maxlength="5" placeholder="e.g. TECH" style="text-transform: uppercase" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Company Name</label>
                <input v-model="formData.name" type="text" required placeholder="e.g. TechCorp Inc." class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-300 mb-2">Description</label>
                <textarea v-model="formData.description" rows="3" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" placeholder="Company description..."></textarea>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Sector</label>
                <select v-model="formData.sector" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                  <option v-for="opt in stockSectors" :key="opt.name" :value="opt.name">{{ opt.label }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Current Price</label>
                <input v-model.number="formData.current_price" type="number" required min="0.01" step="0.01" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Shares Available</label>
                <input v-model.number="formData.shares_available" type="number" required min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Volatility (%)</label>
                <input v-model.number="formData.volatility" type="number" required min="0" max="100" step="0.1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Active Trading</label>
                <div class="flex items-center gap-2">
          ...
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Stock</button>
          </div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  try { stockSectors.value = (await api.get('/admin/stock-sectors')).data } catch(e) {}
  fetchItems()
})
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/stocks/stocks', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/stocks/stocks/${editingItem.value.id}`, formData.value)
      toast.success('Stock updated successfully!')
    } else {
      await api.post('/admin/stocks/stocks', formData.value)
...
const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Stock? This action cannot be undone.', 'Delete Stock')
  if (!confirmed) return
  try {
    await api.delete(`/admin/stocks/stocks/${item.id}`)
