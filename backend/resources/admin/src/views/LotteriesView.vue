  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="relative flex-1 max-w-md">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search lotteries..." @input="debouncedSearch" class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all" />
      </div>
        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Ticket Price</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Sold</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Max</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Prize Pool</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Status</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Draw Date</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.ticket_price || 0).toFixed(2)}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.tickets_sold }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.max_tickets }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.prize_pool || 0).toFixed(2)}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.status }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.draw_date ? new Date(item.draw_date).toLocaleString() : '-' }}</td>
              <td class="px-6 py-4">
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Lottery</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-300 mb-2">Lottery Name</label>
                <input v-model="formData.name" type="text" required placeholder="e.g. Daily Jackpot" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Ticket Price</label>
                <input v-model.number="formData.ticket_price" type="number" required min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Max Tickets</label>
                <input v-model.number="formData.max_tickets" type="number" min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Prize Pool</label>
                <input v-model.number="formData.prize_pool" type="number" min="0" readonly class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                <p class="text-xs text-slate-400 mt-1">Auto-calculated from ticket sales</p>
              </div>
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-300 mb-2">Draw Date</label>
                <input v-model="formData.draw_date" type="datetime-local" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Status</label>
                <select v-model="formData.status" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                  <option v-for="opt in lotteryStatuses" :key="opt.name" :value="opt.name">{{ opt.label }}</option>
                </select>
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Lottery</button>
          </div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  try { lotteryStatuses.value = (await api.get('/admin/lottery-statuses')).data } catch(e) {}
  fetchItems()
})
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/casino/lotteries', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/casino/lotteries/${editingItem.value.id}`, formData.value)
      toast.success('Lottery updated successfully!')
    } else {
      await api.post('/admin/casino/lotteries', formData.value)
      toast.success('Lottery created successfully!')
    }
    closeModal()
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to save Lottery')
  }
}

const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Lottery? This action cannot be undone.', 'Delete Lottery')
  if (!confirmed) return
  try {
    await api.delete(`/admin/casino/lotteries/${item.id}`)
    toast.success('Lottery deleted successfully!')
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to delete Lottery')
  }
}
