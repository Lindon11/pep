  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="relative flex-1 max-w-md">
        ...
        <input v-model="searchQuery" type="text" placeholder="Search users..." @input="debouncedSearch" class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all" />
      </div>
        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Last Active</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.id }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.username }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.email }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.level }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.cash || 0).toFixed(2)}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.currentRank?.name || '-' }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.last_active ? new Date(item.last_active).toLocaleString() : '-' }}</td>
              <td class="px-6 py-4">
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} User</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                ...
                <input v-model="formData.username" type="text" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Email</label>
                <input v-model="formData.email" type="email" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                ...
                <input v-model="formData.password" type="password" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                <p class="text-xs text-slate-400 mt-1">Leave blank when editing to keep current password</p>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Cash</label>
                <input v-model.number="formData.cash" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Bank Balance</label>
                <input v-model.number="formData.bank" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Level</label>
                <input v-model.number="formData.level" type="number" min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save User</button>
          </div>
        </div>
      </div>
    </div>
