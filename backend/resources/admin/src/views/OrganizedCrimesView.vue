        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Success Rate</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Min Reward</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Max Reward</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.required_members }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `${parseFloat(item.success_rate).toFixed(1)}%` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.min_reward).toLocaleString()}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.max_reward).toLocaleString()}` }}</td>
              <td class="px-6 py-4">
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Organized Crime</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Crime Name</label>
              ...
              <div>
                ...
                <input v-model.number="formData.success_rate" type="number" required min="0" max="100" step="0.01" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Min Reward ($)</label>
                <input v-model.number="formData.min_reward" type="number" required step="0.01" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Max Reward ($)</label>
                <input v-model.number="formData.max_reward" type="number" required step="0.01" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Required Members</label>
                <input v-model.number="formData.required_members" type="number" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                ...
                <input v-model.number="formData.cooldown" type="number" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Organized Crime</button>
          </div>
        </div>
      </div>
    </div>
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/organized-crimes', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/organized-crimes/${editingItem.value.id}`, formData.value)
      toast.success('Organized Crime updated successfully!')
    } else {
      await api.post('/admin/organized-crimes', formData.value)
      toast.success('Organized Crime created successfully!')
    }
    closeModal()
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to save Organized Crime')
  }
}

const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Organized Crime? This action cannot be undone.', 'Delete Organized Crime')
  if (!confirmed) return
  try {
    await api.delete(`/admin/organized-crimes/${item.id}`)
    toast.success('Organized Crime deleted successfully!')
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to delete Organized Crime')
  }
}
