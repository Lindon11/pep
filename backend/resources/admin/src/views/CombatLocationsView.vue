        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Min Level</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Order</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.min_level }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.order }}</td>
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Combat Location</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div class="col-span-2">
                ...
                <input v-model="formData.name" type="text" required placeholder="e.g. ARCADE" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div class="col-span-2">
              ...
              <div>
                ...
                <input v-model="formData.icon" type="text" maxlength="10" placeholder="🎮" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Minimum Level</label>
                <input v-model.number="formData.min_level" type="number" min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm text-slate-400">Enable this location</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Combat Location</button>
          </div>
        </div>
      </div>
    </div>
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/combat-locations', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/combat-locations/${editingItem.value.id}`, formData.value)
      toast.success('Combat Location updated successfully!')
    } else {
      await api.post('/admin/combat-locations', formData.value)
      toast.success('Combat Location created successfully!')
    }
    closeModal()
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to save Combat Location')
  }
}

const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Combat Location? This action cannot be undone.', 'Delete Combat Location')
  if (!confirmed) return
  try {
    await api.delete(`/admin/combat-locations/${item.id}`)
    toast.success('Combat Location deleted successfully!')
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to delete Combat Location')
  }
}
