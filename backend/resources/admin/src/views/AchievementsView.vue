        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Requirement</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.requirement }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${(item.reward_cash || 0).toLocaleString()}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.reward_xp }}</td>
              <td class="px-6 py-4">
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Achievement</h2>
            ...
            <div class="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select v-model="formData.type" required class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                  <option v-for="opt in achievementStats" :key="opt.name" :value="opt.name">{{ opt.label }}</option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Requirement</label>
                <input v-model.number="formData.requirement" type="number" required min="1" placeholder="Target number" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Cash Reward</label>
                <input v-model.number="formData.reward_cash" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">XP Reward</label>
                <input v-model.number="formData.reward_xp" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Icon</label>
                <input v-model="formData.icon" type="text" placeholder="🏆" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Sort Order</label>
                <input v-model.number="formData.sort_order" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Achievement</button>
          </div>
        </div>
      </div>
    </div>
...
onMounted(async () => {
  try { achievementStats.value = (await api.get('/admin/achievement-stats')).data } catch(e) {}
  fetchItems()
})
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/achievements', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/achievements/${editingItem.value.id}`, formData.value)
      toast.success('Achievement updated successfully!')
    } else {
      await api.post('/admin/achievements', formData.value)
...
const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Achievement? This action cannot be undone.', 'Delete Achievement')
  if (!confirmed) return
  try {
