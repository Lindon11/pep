        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Success %</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Min Value</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Max Value</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Req. Level</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              ...
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.success_rate }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${item.min_car_value?.toLocaleString() || 0}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${item.max_car_value?.toLocaleString() || 0}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.required_level }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.cooldown }}</td>
              <td class="px-6 py-4">
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Theft Type</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Success Rate (%)</label>
                <input v-model.number="formData.success_rate" type="number" required min="1" max="100" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Required Level</label>
                <input v-model.number="formData.required_level" type="number" required min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Min Car Value</label>
                <input v-model.number="formData.min_car_value" type="number" required min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Max Car Value</label>
                <input v-model.number="formData.max_car_value" type="number" required min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Max Damage %</label>
                <input v-model.number="formData.max_damage" type="number" min="0" max="100" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Jail Multiplier</label>
                <input v-model.number="formData.jail_multiplier" type="number" min="0" step="0.1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Cooldown (seconds)</label>
                <input v-model.number="formData.cooldown" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
            </div>
          </div>
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Theft Type</button>
          </div>
        </div>
      </div>
    </div>
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/theft-types', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/theft-types/${editingItem.value.id}`, formData.value)
      toast.success('Theft Type updated successfully!')
    } else {
      await api.post('/admin/theft-types', formData.value)
      toast.success('Theft Type created successfully!')
    }
    closeModal()
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to save Theft Type')
  }
}

const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Theft Type? This action cannot be undone.', 'Delete Theft Type')
  if (!confirmed) return
  try {
    await api.delete(`/admin/theft-types/${item.id}`)
    toast.success('Theft Type deleted successfully!')
    fetchItems()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Failed to delete Theft Type')
  }
}
