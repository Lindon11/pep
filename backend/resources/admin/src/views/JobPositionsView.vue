        ...
        <table class="w-full">
          <thead class="bg-slate-700/50 border-b border-slate-600/50">
            <tr>
              ...
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Company ID</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Salary</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Level Req.</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Employees</th>
              <th class="px-6 py-4 text-left text-sm font-semibold text-slate-300">Max</th>
          ...
          <tbody class="divide-y divide-slate-700/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-slate-700/25 transition-colors">
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.title }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.company_id }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ `$${parseFloat(item.base_salary || 0).toFixed(2)}` }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.required_level }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.current_employees }}</td>
              <td class="px-6 py-4 text-sm text-slate-300">{{ item.max_employees }}</td>
      ...
      <div class="flex items-center justify-center min-h-screen p-4">
        ...
        <div class="relative bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div class="sticky top-0 bg-slate-800 border-b border-slate-700 p-6 flex items-center justify-between">
            <h2 class="text-xl font-bold text-white">{{ editingItem ? 'Edit' : 'Create' }} Job Position</h2>
          ...
          <div class="p-6">
            <div class="grid grid-cols-2 gap-6">
              <div class="col-span-2">
                ...
                <input v-model="formData.title" type="text" required placeholder="e.g. Software Engineer" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div class="col-span-2">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Company ID</label>
                <input v-model.number="formData.company_id" type="number" required min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Base Salary</label>
                <input v-model.number="formData.base_salary" type="number" required min="0" placeholder="e.g. 5000" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Required Level</label>
                <input v-model.number="formData.required_level" type="number" min="1" max="100" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Required Intelligence</label>
                <input v-model.number="formData.required_intelligence" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Required Endurance</label>
                <input v-model.number="formData.required_endurance" type="number" min="0" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Max Employees</label>
                <input v-model.number="formData.max_employees" type="number" min="1" class="w-full px-4 py-2.5 bg-slate-700/50 border-2 border-slate-600/50 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
              </div>
              <div>
          ...
          <div class="sticky bottom-0 bg-slate-800 border-t border-slate-700 p-6 flex items-center gap-3 justify-end">
            ...
            <button @click="saveItem" class="px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-600 text-white font-medium hover:from-amber-600 hover:to-orange-700 transition-all shadow-lg shadow-amber-500/25">Save Job Position</button>
          </div>
        </div>
      </div>
    </div>
...
const fetchItems = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await api.get('/admin/employment/positions', { params: { page, search: searchQuery.value } })
...
const saveItem = async () => {
  try {
    if (editingItem.value) {
      await api.patch(`/admin/employment/positions/${editingItem.value.id}`, formData.value)
      toast.success('Job Position updated successfully!')
    } else {
      await api.post('/admin/employment/positions', formData.value)
...
const deleteItem = async (item) => {
  const confirmed = await confirm.confirm('Are you sure you want to delete this Job Position? This action cannot be undone.', 'Delete Job Position')
  if (!confirmed) return
  try {
    await api.delete(`/admin/employment/positions/${item.id}`)
